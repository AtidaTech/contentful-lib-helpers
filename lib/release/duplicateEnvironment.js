...
export async function duplicateEnvironment(
  ...
  destinationEnvironmentId = 'duplicate',
  verbosityLevel = 1
) {
  if (
  ...
  verbosityLevel > 2 &&
    console.log(
      `##/INFO: Duplicating environment '${sourceEnvironmentId}' for space '${space?.sys?.id}'`
    )
      ...
      if (
        ...
        await space
          ...
          .catch(error => {
            ...
            verbosityLevel > 0 &&
              console.error(
                "@@/ERROR: It might be that you don't have any free environment slot to create a new one!"
              )
            process.exit(1)
          })
        verbosityLevel > 2 &&
